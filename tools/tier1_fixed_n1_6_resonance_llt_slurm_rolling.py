"""Entrypoint for the fixed-N1=6 Tier-1 resonance/Llt search lane."""
...
if _existing not in {None, VARIANT}:
    raise RuntimeError(
        "MFT_TIER1_SEARCH_VARIANT conflicts with fixed-N1=6 entrypoint: "
        f"{_existing}"
    )
...
if (
    ...
    or rolling.OPTIMIZER_RESONANCE_SCALE_HZ != 100.0
    or rolling.OPTIMIZER_CORE_THERMAL_SCALE_C != 4.0
    or rolling.SEED_START != 1_907_196_000
    or rolling.ROLLING_TARGET != 32
    or rolling.POPULATION != 320
    or rolling.MAX_GENERATIONS != 600
    or rolling.INFERENCE_THREADS != 8
    or rolling.TASK_PRIORITY != -3
    or rolling.LEGACY_TASK_PRIORITY != -4
    or rolling.PRIORITY_MIGRATION_SEED_CUTOFF != 1_907_196_000
    or rolling.TASK_NAME_STEM != "mft-t1n6res"
