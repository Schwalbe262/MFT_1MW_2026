"""Entrypoint for the fixed-N1=5 Tier-1 thermal/Llt search lane."""
...
VARIANT = "fixed-n1-5-thermal-llt-v1"
_existing = os.environ.get("MFT_TIER1_SEARCH_VARIANT")
if _existing not in {None, VARIANT}:
    raise RuntimeError(
        "MFT_TIER1_SEARCH_VARIANT conflicts with fixed-N1=5 entrypoint: "
        f"{_existing}"
    )
...
if (
    ...
    or rolling.SEED_START != 1_907_195_000
    or rolling.ROLLING_TARGET != 32
    or rolling.POPULATION != 320
    or rolling.MAX_GENERATIONS != 600
    or rolling.INFERENCE_THREADS != 8
    or rolling.TASK_PRIORITY != -4
    or rolling.LEGACY_TASK_PRIORITY != -5
    or rolling.PRIORITY_MIGRATION_SEED_CUTOFF != 1_907_195_000
    or rolling.TASK_NAME_STEM != "mft-t1n5therm"
