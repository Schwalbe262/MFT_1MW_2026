...
프로젝트의 versioned `simulation-policy`에서 desired simulation 수를 drain 방식으로
바꾸는 CAS PATCH다. 프로젝트의 `max_active_tasks` 안전 상한은 demand로 사용하지 않는다.
...
병렬 목표 변경 API는 loopback 또는 `192.168.0.0/24` client, 명시적 Host allowlist,
same-origin, JSON 전용 custom header를 모두 검사한다. 신뢰 LAN에서 열려면 실제 접속에
사용할 Host/IP를 지정해 다음처럼 실행한다. 인터넷 또는 reverse proxy 공개는 지원하지
않는다.

```powershell
.\start_monitor.ps1 -ListenAddress 0.0.0.0 -OperatorHosts 192.168.0.37,monitor.local
```
