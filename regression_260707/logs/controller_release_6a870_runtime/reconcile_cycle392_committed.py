"""Review and seal the accepted prefix of interrupted refill cycle 392."""
...
EVIDENCE_PATH = RUNTIME_ROOT / "cycle392_committed_evidence.json"
...
def _load_cycle() -> dict:
    ...
    if not isinstance(cycle, dict):
        raise RuntimeError("cycle 392 is missing")
    return cycle
...
def _accepted_prefix(cycle: dict) -> tuple[list[dict], list[dict]]:
    ...
    if not (
        ...
        raise RuntimeError("cycle 392 is not the exact observed accepted-prefix failure")
...
def _scheduler_rows(accepted: list[dict]) -> list[dict]:
    response = requests.get(
        ...
        params={"limit": 1000, "name_prefix": PREFIX},
        timeout=60,
    )
    ...
    for event in accepted:
        ...
        if task is None:
            raise RuntimeError(f"scheduler readback missing accepted task {task_id}")
        checks = {
    ...
    accepted_names = {event["name"] for event in accepted}
    planned_unaccepted = {
        event["name"] for event in _load_cycle()["formal_journal"]["events"]
        if event.get("task_id") is None
    }
    scheduler_names = {str(task.get("name") or "") for task in tasks}
    if accepted_names & planned_unaccepted or scheduler_names & planned_unaccepted:
        raise RuntimeError("an unaccepted planned identity exists in the scheduler")
    return rows


def _replay_target(before: dict, events: list[dict], accepted: list[dict]) -> dict:
    ...
    if first_serial != int(before["serial"]) + 1 or last_serial != 19525:
...
def build() -> dict:
    ...
    evidence = {
        ...
        "unaccepted_count": len(events) - len(accepted),
...
def publish(reviewed_sha: str) -> dict:
    ...
    # Commit the feeder ledger first.  A crash after this point remains safely
    # detectable; sealing the cycle first could permit candidate replay.
    ...
    with FileLock(str(LOCK_ROOT / "controller-loop.lock"), timeout=0):
        with scheduler_client.campaign_mutation_lock(timeout=30):
            if not args.publish:
                ...
                return 0
            result = publish(str(args.reviewed_sha or ""))
